...
-- Changes: #3 (SSU misclassification), #4 (success outside 744h window),
--          #5 (DLQ record in wrong bucket), #6 (no file URL bug ticket)
-- ============================================================

-- STEP 1: Entity breakdown of the 224 vendors
SELECT
  global_entity_id,
...
mds_not_received AS (
  ...
  FROM `fulfillment-dwh-production.curated_data_shared_vendor.fact_vso_vrm_mds_menu_funnel`
  WHERE management_entity = 'Talabat' AND account_source_curated = 'Non SSU'
    AND onboard_month = '2026-04-01' AND is_funnel_mds_not_received = TRUE
),
mds_success AS (
...
-- ============================================================
-- STEP 4: Extract NO FILE URL vendors for bug ticket (Change #6)
-- ============================================================
WITH
mds_not_received AS (
  SELECT management_entity, global_entity_id, grid_id, vendor_id, onboarded_date,
